<template>
  <div class="pagin">
    ...
    <ul class="pager" @click="handlePagerClick">
      ...
      <li v-if="showPrevMore" class="page-more"></li>
      <li
      ...
      <li v-if="showNextMore" class="page-more"></li>
      <li
      v-if="pageCount > 1"
...
export default defineComponent({
  ...
  setup (props, { emit }) {
    ...
    const pagers = computed(() => {
      ...
      // 页面按钮数的一般， 判断是否需要显示省略号
      const halfPagerCount = (pagerCount - 1) / 2
      let showPrevMore = false
      let showNextMore = false

      if (pageCount > pagerCount) {
        // 当越过一半中心点从新以 currentPage为中心点，这时候就得显示前...
      ...
      const array = []
      // 前省略号存在而没有后省略号的情况，即中间的点就算最后的点
      if (showPrevMore && !showNextMore) {
        // pagerCount - 2 除去前后两个点，剩下的就是所有的点减去5 个点的起始点开始
