<template>
  ...
  <ul>
    <li class="list-item" v-for="(song, index) in songs" :key="song.id" @dblclick="handlePlaySong(index)">
      <i v-if="currentSongInfo.currentSong && currentSongInfo.currentSong.id === song.id && currentSongInfo.currentIndex === index" class="item-index iconfont icon-song-volum"></i>
      <span v-else class="item-index">{{ (index + 1).toString().padStart(2, "0") }}</span>
...
.list-item {
  ...
  &.list-header {
    background:  #fff;
    .item-title, .item-singer, .item-album {
      color: #888;
    }
  }
}
