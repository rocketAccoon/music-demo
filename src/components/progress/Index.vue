...
<script lang="ts">
import { defineComponent, PropType, ref, reactive, watchEffect, toRefs } from 'vue'
...
export default defineComponent({
  ...
  setup (props) {
    ...
    const _offset = (moveWidth: number) => {
      if (!progressRef.value || !progressBarRef.value) {
        return
      }
    ...
    /* 点击事件，真实移动长度 moveWidth = 点击的时候距离页面的距离 - 灰色条距离左边的长度 */
    ...
    watchEffect(() => {
      ...
      if (percent.value >= 0 && percent.value <= 1 && !touch.initial && progressBarRef.value) {
